...
function ViewSinglePost() {
  ...
  useEffect(() => {
    ...
    async function fetchPost() {
      try {
        const response = await Axios.get(`/post/${id}`, { signal: controller.signal })
  ...
  async function deleteHandler() {
    ...
    if (areYouSure) {
      try {
        ...
        console.log("there was a aproblem")
      }
    }
  }
  return (
    <Page title={post.title}>
      ...
      <p className="text-muted small mb-4">
        ...
        Posted by <Link to={`/profile/${post.author.username}`}>{post.author.username}</Link> {dateFormated}
      </p>

      <div className="body-content">
        <RecatMarkdown children={post.body} allowedElements={["p", "br", "strong", "em", "h1", "h2", "h3", "h4", "h5", "h6", "ul", "ol", "li"]} />
      </div>
    </Page>
  )
}
